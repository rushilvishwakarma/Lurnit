...
export default function HomePage() {
  return (
    <ContentLayout title="Previous Year Questions">
      ...
      <div className="grid grid-cols-3 sm:grid-cols-3 md:grid-cols-5 lg:grid-cols-7 gap-7 w-full py-10">
        {links.map((link, index) => (
          <Link key={index} href={link.href}>
            <div className="flex flex-col gap-2 hover:opacity-75 cursor-pointer">
              <img
                src={link.image}
                className="bg-muted rounded-md aspect-square mb-4 px-2 py-2"
              />
